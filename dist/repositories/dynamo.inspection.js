...
export async function insertInspectionService({ text, attachments, name, }) {
    ...
    const photos = attachments?.map((a) => a.url || a.thumbnail || a.id).filter(Boolean) || [];
    ...
    const item = {
        ...
        seatbeltDoorsMirrors: {
            BOOL: getAnswer("Are the Seatbelts, Doors and Mirror's Functioning Correctly?"),
        },
        ...
        spareTyre: {
            BOOL: getAnswer('Is there a Spare tyre, jack, Spanner on the vehicle and in good condition?'),
        },
        numberPlate: {
            BOOL: getAnswer('Is there a valid number plate on the Front and Back of the vehicle?'),
        },
        ...
        lights: {
            BOOL: getAnswer('Are the headlights, Taillights, Fog Lights, indicators and hazards functioning correctly?'),
        },
        defrosterAircon: {
            BOOL: getAnswer('Are the defrosters, heaters and air conditioners functional?'),
        },
        ...
        siteKit: {
            BOOL: getAnswer('Is there Reflectors, Buggy Whip, Strobe Light within the Vehicle?'),
        },
