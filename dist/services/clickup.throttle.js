...
import { client as redisClient } from "./redis.service.js";
 ...
 */
async function acquireClickUpSlot() {
    ...
    while (true) {
        ...
        try {
            ...
            if (ttlSeconds > 0)
                waitMs = ttlSeconds * 1000;
        }
        catch {
            // Redis ttl() failed - fall back to waiting a full window.
        }
        logger.warn(`[ClickUp throttle] Budget (${limit}/min) reached, waiting ${waitMs}ms for window reset`);
 ...
 */
export async function clickupFetch(url, init) {
