...
import { getQuoteByNumber, createQuote, updateQuote, } from "../repositories/dynamo.quote.repository.js";
...
export async function syncQuoteToCrmShaughn(quote) {
...
async function handleCrmShaughnTask(quote, action) {
    const payload = buildShaughnPayload(action, quote);
    if (!quote.clickUpTaskidCrmShaughn) {
        const task = await createClickUpTask(payload.topic, payload.description, payload.listid, payload.status, payload.customFields, payload.due_date);
        if (!task) {
        ...
        if (payload.comment)
            await addClickUpComment(task.id, payload.comment);
        logger.info(`[CRM Shaughn] Created task ${task.id} for ${quote.quoteNumber}`);
        return;
    }
    // Task already exists — update in place
    await updateClickUpTask(quote.clickUpTaskidCrmShaughn, payload.topic, payload.description, payload.status, payload.customFields);
    if (payload.comment)
        await addClickUpComment(quote.clickUpTaskidCrmShaughn, payload.comment);
    logger.info(`[CRM Shaughn] Updated task ${quote.clickUpTaskidCrmShaughn} (${action})`);
}
function buildShaughnPayload(action, quote) {
...
// ─── ClickUp helpers ──────────────────────────────────────────────────────────
async function createClickUpTask(topic, description, listId, status, customFields, due_date) {
...
async function updateClickUpTask(taskId, topic, description, status, customFields) {
