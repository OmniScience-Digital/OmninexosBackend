...
import { getddHrdTasks } from "../../repositories/dynamo.hrd.repository.js";
import { getddFleetTasks } from "../../repositories/dynamo.task.js";
import { dynamoDBTaskSchema, dynamoDBEmployeeTaskSchema, } from "../../schema/task.schema.js";
...
export const getHrdTasks = async () => {
    try {
        const tasks = await getddHrdTasks();
        const { validItems, invalidItems } = await validateAndFilter(dynamoDBEmployeeTaskSchema)(tasks ?? []);
        logger.info(`Task DynamoDB Valid items: ${validItems.length}`);
        logger.warn(`Task DynamoDB Invalid items: ${invalidItems.length}`);
        // Log details of invalid items
        invalidItems.forEach(({ item, error }, index) => {
