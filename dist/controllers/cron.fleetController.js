...
const fleetHelper = async (items, tasks) => {
    ...
    for (let i = 0; i < items.length; i++) {
        try {
            ...
            // Comparison Logic
            const isBreakAndLuxExpired = breakAndLux && jhbCurrentDate > new Date(breakAndLux.getTime() + 60 * 24 * 60 * 60 * 1000);
            const isLicenseDiscExpired = licenseDisc && jhbCurrentDate > new Date(licenseDisc.getTime() + 334 * 24 * 60 * 60 * 1000);
