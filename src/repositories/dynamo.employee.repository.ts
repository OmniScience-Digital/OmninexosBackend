...
export async function insertEmployeeTaskService({
  employeeId,
  employeeName,
  taskType,
  documentType,
  ...
  if (employeeName) item.employeeName = { S: employeeName };
  if (taskType) item.taskType = { S: taskType };
  if (documentType) item.documentType = { S: documentType };
  if (documentIdentifier) item.documentIdentifier = { S: documentIdentifier };
  if (clickupTaskId) item.clickupTaskId = { S: clickupTaskId };

  await dynamoClient.send(new PutItemCommand({ TableName: EMPLOYEE_TASK_TABLE, Item: item }));

  logger.info(
    `Employee task created for ${employeeId} (${taskType ?? 'N/A'}) document: ${documentType ?? 'N/A'}`
  );
}
...
export async function deleteEmployeeTask({
  ...
  try {
    ...
    if (!result.Items || result.Items.length === 0) {
      logger.warn(
        `No employee task found for employeeId: ${employeeId}, documentIdentifier: ${documentIdentifier}`
      );
      return null;
    }
