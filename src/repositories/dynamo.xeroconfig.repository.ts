...
export async function acquireXeroLock(
  ...
  if (!config) throw new Error('No Xero config found in Database');
...
export async function releaseXeroLock(tenantId: string, lockValue: string): Promise<void> {
  const config = await getXeroConfig(tenantId);
  if (!config) return;
  ...
  try {
    ...
    if (err instanceof ConditionalCheckFailedException) {
      logger.warn(
        `[XeroConfig] Lock already released/expired/reacquired for tenant ${tenantId} - ignoring.`
      );
      return;
    }
    throw err;
