...
<<<<<<< HEAD
  ...
  const additionals = allAdditionals.filter(
  ...
  logger.info(
    `Filtered ${additionals.length} of ${allAdditionals.length} additionals for this compliance`
  );
  ...
  for (const employee of relevantEmployees) {
    ...
    const employeeName =
      extractValue(employee.employee.knownAs) ||
...
<<<<<<< HEAD
  ...
  for (const additional of additionals) {
    ...
    const isCritical =
      extractValue(additional.critical) === 'true' ||
  ...
  logger.info(`--- TOTALS ---`);
  logger.info(
    `Total Requirements: ${totalRequirements} (${relevantEmployees.length} employees + ${additionals.length} site docs)`
  );
  ...
  logger.info(
    `Valid for 30-Day (total - expired - expiring): ${totalRequirements - expiredRequirements - expiringIn30Days}`
  );
  ...
  if (totalRequirements > 0) {
    ...
    logger.info(
      `Valid for Current: ${totalRequirements} - ${expiredRequirements} = ${validForCurrent}`
    );
    logger.info(
      `Valid for 30-Day: ${totalRequirements} - ${expiredRequirements} - ${expiringIn30Days} = ${validFor30Days}`
    );
  ...
  logger.info(
    `Current: ${ratingBeforePenalty}% - ${additionalPenalty * 100}% = ${complianceRating}%`
  );
  logger.info(
    `30-Day: ${rating30BeforePenalty}% - ${additionalPenalty * 100}% = ${complianceRating30Days}%`
  );
