...
import { DynamoDBComplianceAdditional, DynamoDBComplianceRecord, DynamoDBCustomerSite } from "../schema/crm.schema"
...
const processAllCompliances = (
    ...
    return compliances.map(compliance => {
        ...
        const complianceAdditionals = allAdditionals.filter(additional =>
            extractValue(additional.complianceid) === complianceId
        );

        const ratings = calculateComplianceRatings(
            compliance,
            allEmployees,
...
function checkExistingComplianceTask(
    ...
    return tasks.some(task =>
        extractValue(task.documentIdentifier) === documentIdentifier
    );
}
...
function getSiteName(customerSiteId: string, customerSites: DynamoDBCustomerSite[]): string {
    ...
    const site = customerSites.find(site =>
        extractValue(site.id) === customerSiteId
    );
    ...
    try {
        ...
        for (const result of results) {
            try {
                ...
                const complianceAdditionals = additionals.filter(additional =>
                    extractValue(additional.complianceid) === result.complianceId
                );
