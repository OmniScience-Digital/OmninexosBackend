 ...
 */
export const xeroQuoteWebhookController = async (req: Request, res: Response) => {
  try {
...
async function processQuoteWebhookEvents(payload: XeroWebhookPayload) {
...
async function handleQuoteEvent(event: XeroWebhookEvent) {
  try {
    ...
    if (!res.ok) throw new Error(`Xero quote fetch failed: ${res.status} ${res.statusText}`);
