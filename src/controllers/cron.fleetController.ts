...
export const FleetController = async (fleets: DynamoDbFleetItem[], tasks: DynamoDbTaskItem[]) => {
  try {
...
const fleetHelper = async (items: DynamoDbFleetItem[], tasks: DynamoDbTaskItem[]) => {
    ...
    try {
      ...
      if (isLicenseDiscExpired) {
        ...
        if (!licenseDiscTaskExists) {
          ...
          if (!task) {
            console.error(
              `Failed to create ClickUp task for ${vehicle.vehicleReg?.S} - licensedisc`
            );
            continue;
          }
...
async function createTasks(descriptionLines: string, topic: string, username: string) {
